	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		...
		if (entity instanceof Villager) {
			if (((Villager) entity).getCustomName().equalsIgnoreCase(
					ChatColor.GREEN + "Bob")
					|| ((Villager) entity).getCustomName().equalsIgnoreCase(
		...
		if (event.getDamager() instanceof Arrow) {
			...
			if (shooter instanceof Player) {
				if (event.getEntity() == shooter) {
					if ((((Player) shooter).getItemInHand().getType() == Material.BOW)
		...
		if (event.getEntity() instanceof Player) {
			...
			if (event.getDamager() instanceof Player) {
				...
				if ((plugin.fisher.contains(damager.getName())
						&& (plugin.fished.contains(player.getName())) || (plugin.fisher
						.contains(player.getName()))
						&& (plugin.fished.contains(damager.getName())))) {
					event.setCancelled(true);
				}
			}
		}
		if ((event.getDamager() instanceof Player)
			...
			double angle = Math
					.abs(Math.toDegrees(player.getEyeLocation().getDirection()
							.angle(player.getLocation().getDirection())));

			if (angle < 45.0D) {
				if ((damager.getItemInHand().getType() == Material.IRON_SWORD)
						&& (damager.getItemInHand().getItemMeta()
								.getDisplayName()
		...
		if (((event.getDamager() instanceof Player))
			...
			if (!plugin.inCombat.contains(damager)) {
				...
				// damager.sendMessage(plugin.titleZP + ChatColor.RED +
				// "You are now in combat with " + entity.getName());
				// player.sendMessage(plugin.titleZP + ChatColor.RED +
				// "You are now in combat with " + damager.getName());
				...
				this.plugin
						...
						.runTaskLaterAsynchronously(
								...
								new Runnable() {
									...
									@Override
									public void run() {
										// damager.sendMessage(plugin.titleZP +
										// ChatColor.RED +
								...
								plugin.cm.getCombatConfig().getLong(
										"Time_In_Combat") * 20L);
			}
		}
		if (((event.getDamager() instanceof Arrow))
			...
			if (((proj instanceof Arrow)) && ((shooter instanceof Player))) {
				...
				if (!plugin.inCombat.contains(attacker)) {
					...
					// attacker.sendMessage(plugin.titleZP + ChatColor.RED +
					// "You are now in combat with " + entity.getName());
					// player.sendMessage(plugin.titleZP + ChatColor.RED +
					// "You are now in combat with " + attacker.getName());
					...
					plugin.getServer()
							...
							.runTaskLaterAsynchronously(
									...
									new Runnable() {
										...
										@Override
										public void run() {
											// attacker.sendMessage(plugin.titleZP
											// + ChatColor.RED +
											// "You are no longer in combat!");
											// player.sendMessage(plugin.titleZP
											// + ChatColor.RED +
		...
		if ((event.getEntity() instanceof Player)
				&& (event.getDamager() instanceof Fish)) {
			Player player = (Player) event.getEntity();
	...
	public void onEntityDamage(EntityDamageEvent event) {
		if (event.getEntity() instanceof Player) {
			...
			if (plugin.noFall.contains(((Player) event.getEntity()).getName())) {
				if (event.getCause() == DamageCause.FALL) {
					plugin.noFall
							.remove(((Player) event.getEntity()).getName());
		...
		if (entity instanceof Arrow) {
			...
			if (shooter instanceof Player) {
				...
				if ((player.getItemInHand().getType() == Material.BOW)
					...
					player.getWorld().playSound(arrow.getLocation(),
							Sound.DIG_GRAVEL, 1, 1);
				}
				if ((player.getItemInHand().getType() == Material.BOW)
						&& (player.getItemInHand().getItemMeta()
								.getDisplayName()
