	...
	public static void sendReportsToAdmin(CommandSender sender, String s) {
		for (Player p : Bukkit.getOnlinePlayers())
			if ((p.hasPermission("report.read")) || (p.isOp()))
				p.sendMessage(ChatColor.DARK_GRAY + "[" + ChatColor.DARK_RED
						+ "REPORT" + ChatColor.DARK_GRAY + "] "
						+ ChatColor.GOLD + sender.getName() + " : " + s);
	}

	public static void sendHelp(CommandSender sender) {
		sendMessage(sender, head);

		sendMessage(sender, space + ChatColor.YELLOW
				+ "/report - Report player/issue to admins!");
		sendMessage(sender, space + ChatColor.YELLOW
				+ "/read - Read all reports.");
		sendMessage(sender, space + ChatColor.YELLOW
				+ "/read clear - Clear all reports!");
		sendMessage(sender, head);
	}
}
